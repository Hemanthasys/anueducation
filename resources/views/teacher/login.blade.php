...
<head>
    ...
    @php
        $bgImage   = \App\Models\SiteSetting::get('teacher_login_bg');
...
<body>
<div class="login-bg" style="{{ !$bgImage ? 'background: linear-gradient(135deg, var(--color-dark) 0%, var(--color-primary) 100%);' : '' }}">

    @if($bgImage)
        <div class="login-bg-img" style="background-image: url('{{ asset('storage/' . $bgImage) }}');"></div>
        <div class="login-bg-overlay"></div>
    @else
        <div style="position:absolute;inset:0;background:rgba(0,0,0,0.15);z-index:0;"></div>
    @endif
    ...
    <div class="login-card">
        <div class="login-card-header">
            ...
            <p style="font-size:13px;color:rgba(255,255,255,0.7);margin:0;">{{ __('teacher_portal') }}</p>
            {{-- Language switcher --}}
            <div style="display:inline-flex;gap:0;margin-top:14px;border-radius:8px;overflow:hidden;border:1px solid rgba(255,255,255,0.2);">
                <a href="{{ route('portal.lang', 'en') }}"
                    style="padding:5px 14px;font-size:12px;font-weight:600;text-decoration:none;
                    {{ app()->getLocale() === 'en' ? 'background:rgba(255,255,255,0.25);color:#fff;' : 'color:rgba(255,255,255,0.6);' }}">EN</a>
                <a href="{{ route('portal.lang', 'si') }}"
                    style="padding:5px 14px;font-size:12px;font-weight:600;text-decoration:none;
                    {{ app()->getLocale() === 'si' ? 'background:rgba(255,255,255,0.25);color:#fff;' : 'color:rgba(255,255,255,0.6);' }}">සිං</a>
            </div>
        </div>
                ...
                <div style="margin-bottom:24px;">
                    ...
                    <div style="position:relative;">
                        <input type="password" name="password" id="passwordInput" required
                            class="form-input" placeholder="{{ __('enter_password') }}" style="padding-right:44px;">
